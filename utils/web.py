...
from utils.constants import examples_path, prettify_prefix
from utils.convertor import ply_to_obj


def get_examples():
    """
    Find all example scenes in the examples_path directory.
    ...
    for example_dir in sorted(glob.glob(os.path.join(examples_path, "scene*"))):
        scene_name = os.path.basename(example_dir)
        obj_file = os.path.join(example_dir, f"{prettify_prefix}{scene_name}_vh_clean_2.obj")
